...
static void print_ast_formatted(void *tree, int n);
 ...
 * second argument specifies a left-indentation level.
 */
static void print_ast_formatted(void *tree, int n) {
  NodeType ntype;
  char *name;
  ...
  case FUNC_DEF:
    ...
    print_ast_formatted(func_def_body(tree), n+1);
    printf("/* func_def: %s */\n\n", name);
    break;
  ...
  case FUNC_CALL:
    ...
    print_ast_formatted(func_call_args(tree), 0);   /* print the argument list */
    printf(")\n");
    break;
  ...
  case STMT_LIST:
    ...
    while (tree != NULL) {
      ...
      print_ast_formatted(list_hd, n+1);
    }
    indent(indent_amt);
    printf("}\n");
    break;
  ...
  case IF:
    ...
    print_ast_formatted(stmt_if_expr(tree), 0);
    printf("):\n");
    indent(indent_amt); printf("then:\n");
    print_ast_formatted(stmt_if_then(tree), n+1);
    indent(indent_amt); printf("else:\n");
    print_ast_formatted(stmt_if_else(tree), n+1);
    indent(indent_amt);
    printf("end_if\n");
    break;
  ...
  case ASSG:
    ...
    print_ast_formatted(stmt_assg_rhs(tree), 0);
    printf("\n");
    break;

  case WHILE:
    indent(indent_amt); printf("while (");
    print_ast_formatted(stmt_while_expr(tree), 0);
    printf("):\n");
    print_ast_formatted(stmt_while_body(tree), n+1);
    indent(indent_amt);
    printf("end_while\n");
    break;
  ...
  case RETURN:
    indent(indent_amt);
    printf("return: ");
    print_ast_formatted(stmt_return_expr(tree), 0);
    printf("\n");
    break;

  case EXPR_LIST:
    list_tl = expr_list_rest(tree);
    print_ast_formatted(expr_list_head(tree), 0);
    if (list_tl != NULL) {
      printf(", ");
    }
    print_ast_formatted(list_tl, 0);
    break;
  ...
  case GE:
  case GT:
    print_ast_formatted(expr_operand_1(tree), 0);
    printf(" %s ", opname(ntype));
    print_ast_formatted(expr_operand_2(tree), 0);
    break;
